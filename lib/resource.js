...
Resource.prototype.isUpToDate = function() {
  ...
  return (lastUpdated[this.url] && lastUpdated[this.url] > maxAge);
};
...
// one API
Resource.prototype.getReadablePath = function(onDone, _ignoreReadonly) {
  var self = this;
  var readOnly = _ignoreReadonly ? false : readOnly;
  // try to find a shortcut
  if (!guard.isBlocking(self.url)) {
    if (self.type === 'index' && self.exists()) {
      // is this a index file?
      if (readOnly || self.isUpToDate()) {
    ...
    if (self.type === 'tar' && self.exists()) {
      ...
      return Package.getIndex(self.getPackageName(), self.auth, function(err, data) {
        ...
        verify.check(self.exists(), function(err, actualHash) {
          ...
          Cache.junk(self.url);
          self.getReadablePath(onDone, true);
        });
      });
    }
    ...
    if (readOnly) {
      if (!self.exists()) {
        logger.log('Missing cached package. Refetching ' + self.url);
        Cache.junk(self.url);
        return self.getReadablePath(onDone, true);
      }

      self.err = new Error();
      self.err.statusCode = 404;
